...
export default function AddVentaModal({ venta = null, onSave, onCancel }) {
  ...
  const nextNum = isEdit ? venta.sequence : getNextSaleSequence()

  const activeProductions = getActiveProductions().map((p) => ({
    id: p.id,
  ...
  const productions =
    ...
    !activeProductions.some((p) => p.id === venta.productionId)
      ? [
          { id: venta.productionId, label: venta.productionLabel },
          ...activeProductions,
        ]
      : activeProductions
  ...
  return (
    <div className="venta-modal-overlay" role="dialog" aria-modal="true">
      <div className="venta-modal">
        ...
        <p className="venta-modal__subtitle">Venta {nextNum}</p>
