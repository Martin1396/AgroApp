...
export default function ProductionPanel() {
  const [productions, setProductions] = useState(() => getActiveProductions())
  const [modalOpen, setModalOpen] = useState(false)
  const [form, setForm] = useState(EMPTY_FORM)
  const [errors, setErrors] = useState({})
  const [attempted, setAttempted] = useState(false)

  const refresh = () => setProductions(getActiveProductions())
  ...
  const handleNumericChange = (name, value) => {
    ...
    if (attempted) {
      setErrors(validate({ ...form, [name]: digits }))
    }
  }
  ...
  const validate = (data) => {
    ...
    if (
      ...
      const conflicto = findActiveCamasConflict(data.desdeCama.trim(), data.hastaCama.trim())
      if (conflicto) {
        ...
        <div className="production-modal-overlay" role="dialog" aria-modal="true">
          <div className="production-modal">
            ...
            <form className="production-form" onSubmit={handleConfirm} noValidate>
              ...
              <button type="submit" className="production-form__confirm">
                Confirmar
              </button>
            </form>
          </div>
