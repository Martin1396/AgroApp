...
function HistorialProduccion({ onClearRequest }) {
  const historial = getHistorialProductions()
...
function HistorialVentas({ onClearRequest }) {
...
function HistorialReporte() {
  const { cantidadVentas, totalCop, totalUsd, ventasCop, ventasUsd } = getReporteVentasTotales()
...
export default function HistorialPanel() {
  ...
  const handleClearConfirm = () => {
    if (clearAction === 'produccion') clearHistorialProductions()
    if (clearAction === 'ventas') clearHistorialVentas()
    setClearAction(null)
    refresh()
  }
  ...
  return (
    <section className="historial-panel">
      ...
      <div className="historial-panel__content">
        {tab === 'produccion' && (
          <HistorialProduccion onClearRequest={() => setClearAction('produccion')} />
        )}
        {tab === 'ventas' && (
          <HistorialVentas onClearRequest={() => setClearAction('ventas')} />
        )}
        {tab === 'reporte' && <HistorialReporte />}
      </div>

      {clearAction && (
        <SpecialKeyModal
          title="Limpiar historial"
          message={`Ingresa la clave especial para eliminar todo el historial de ${clearAction === 'produccion' ? 'producción' : 'ventas'}.`}
