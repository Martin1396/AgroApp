...
export default function LoginForm({ onSuccess }) {
  ...
  const handleForgotSubmit = async (e) => {
    ...
    try {
      const existing = await findUserByCedula(cedula)
      if (!existing) {
        setForgotErrors({ cedula: 'No existe una cuenta con esta cédula' })
        return
      }
      await updateUserPassword(cedula, forgotForm.newPassword)
      setForgotSuccess(true)
      setForm((prev) => ({ ...prev, cedula, password: '' }))
    } catch {
      setForgotErrors({ cedula: 'No se pudo actualizar la contraseña' })
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const user = await findUser(form.cedula.trim(), form.password)
      ...
      setError(err.message === 'Error interno' ? 'Error del servidor. Revisa que el backend esté activo.' : 'Cédula o contraseña incorrectos.')
    }
  }
